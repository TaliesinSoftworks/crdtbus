...
export function Bus(config) {
    ...
    (async () => (network = await PeerNetwork(agentId, config.topic, config.signalingServerHost, config.iceServers, config.hostTimeoutMs ?? 15000, state.get, networkStatus.pub, whosOnline.pub, updates.pub)))();
    updates.sub((patch) => {
...
integrationTest("reconnection after host disappears", {
    async "client reconnects and syncs with a new peer after host closes"() {
        ...
        const store1 = MemoryStore({ 1: 1 });
        const bus1 = Bus({ topic, agentId: "agent-1", merge, ...store1, signalingServerHost: "drpeer2.onrender.com", hostTimeoutMs });
        const store2 = MemoryStore({ 2: 2 });
        const bus2 = Bus({ topic, agentId: "agent-2", merge, ...store2, signalingServerHost: "drpeer2.onrender.com", hostTimeoutMs });
        await Promise.all([next(bus1.state), next(bus2.state)]);
        ...
        const store3 = MemoryStore({ 3: 3 });
        const bus3 = Bus({ topic, agentId: "agent-3", merge, ...store3, signalingServerHost: "drpeer2.onrender.com", hostTimeoutMs });
...
async function PeerNetwork(agentId, topic, signalingServerHost, iceServers, hostTimeoutMs, data, networkStatus, whosOnline, update) {
    let agent = deadAgent;
